...
export class MigrationMM4 {
    ...
    static async migrateWorld(options = { force: false }) {
        ...
        ui.notifications.info(
            `Mise à jour du système Mutants & Masterminds 4E à la version ${game.system.version}.` +
        ...
        for (let actor of game.actors.contents) {
            try {
                const update = await MigrationMM4._migrationActor(actor, options);
                if (!foundry.utils.isEmpty(update)) {
                    console.log(`Mutants & Masterminds 4E : Migration de l'actor ${actor.name}[${actor._id}]`);
                    await actor.update(update);
                }
            } catch (err) {
                err.message = `Mutants & Masterminds 4E : Echec de la migration du système pour ${actor.name}[${actor._id}]`;
                console.error(err);
            }
        }
        ...
        for (let item of game.items.contents) {
            try {
                const update = MigrationMM4._migrationItems(item, options);
                if (!foundry.utils.isEmpty(update)) {
        ...
        await setSetting('systemVersion', game.system.version);
        ui.notifications.info(
            `Migration du système de Mutants & Masterminds 4E à la version ${game.system.version} terminé!`,
            {
                permanent: true,
            },
        );
    ...
    static async _migrationActor(actor, options = { force: false }) {
        ...
        if (options?.force || MigrationMM4.needUpdate('1.4.0')) {
        ...
        if (options?.force || MigrationMM4.needUpdate('1.4.1')) {
            const attaque = data.attaque;
        ...
        if (options?.force || MigrationMM4.needUpdate('1.10.0')) {
            const attaque = data.attaque;
        ...
        if (options?.force || MigrationMM4.needUpdate('1.16.0')) {
        ...
        if (options?.force || MigrationMM4.needUpdate('1.21.0')) {
            if (data.accessibility === undefined)
                update[`system.accessibility`] = {
                    font: 'Kalam',
                    fontOther: '',
                };
        }

        if (options?.force || MigrationMM4.needUpdate('1.22.0')) {
        ...
        if (options?.force || MigrationMM4.needUpdate('1.26.0')) {
            ...
            if (version < 1) {
                ...
                if (actor.type === 'personnage') {
                    ...
                    for (let a in attacks) {
                        ...
                        if (type !== 'other') {
                            update[`system.attaque.${a}.skill`] =
                                type === 'combatcontact' ? listCombatContact[id]._id : listCombatDistance[id]._id;
        ...
        if (options?.force || MigrationMM4.needUpdate('1.32.0')) {
            for (let item of actor.items) {
                ...
                for (let e of effects) {
                    ...
                    const find = actor.effects.contents.find(
                        (itm) => itm.origin === `Actor.${actor._id}.Item.${item._id}` && itm.name === e.name,
                    );
        ...
        if (options?.force || MigrationMM4.needUpdate('1.32.9')) {
        ...
        if (options?.force || MigrationMM4.needUpdate('1.32.11')) {
            const attaque = data.attaque;
            let actUpdate = [];
            ...
            for (let effect of actor.effects) {
                ...
                const eff = itm.effects.find(
                    (itm) => itm.flags.variante === effect.flags.variante || itm.name === effect.name,
                );
                let itmUpdate = [];
                let toDelete = true;
        ...
        if (options?.force || MigrationMM4.needUpdate('1.39.13')) {
