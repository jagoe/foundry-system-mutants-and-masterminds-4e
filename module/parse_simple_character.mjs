    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        if (line.includes('Total: Abilities:')) {
            ...
            switch (currentSection) {
                case 'skills':
                    if (line.match(/([A-Za-z\s()]+)\s(\d+)\s\(\+(\d+)/)) {
                        // Check if line is a skill
...
function parseModifiers(line, type) {
    ...
    if (modifiersMatch == null || modifiersMatch.length == 0) {
        ...
        if (modifiersMatch) {
            if (
                modifiersMatch[0].includes('Flaws:') ||
                modifiersMatch[0].includes('Extras:') ||
                modifiersMatch[0].includes('Feats:')
            ) {
                return [];
            }
        ...
        for (let i = 0; i < modifiersMatch.length; i++) {
            ...
            if (parts !== null && parts.length > 1) {
                ...
                if (modifiersMatched == true) {
                    ...
                    type === 'extras' &&
                    (modifierValue >= 0 || modifierValue == null || modifierValue == undefined)
                ) {
                    modifiers.push({ name: modifierName, modifier: modifierValue });
                }
            }
    ...
    while ((match = descriptorPattern.exec(line)) !== null) {
        ...
        if (hasValidDescriptor) {
            descriptors = descriptors.concat(
                potentialDescriptors.filter(
                    (desc) => !desc.includes('Flaws:') && !desc.includes('Extras:') && !desc.includes('Feats:'),
                ),
            );
        }
    }
...
function parseTotal(input) {
    ...
    const totalPattern =
        /^Total: Abilities: (\d+) \/ Skills: (\d+)--\d+ \/ Advantages: (\d+) \/ Powers: (\d+)(?: \+ [A-Za-z &/-]+)? \/ Defenses: (\d+) \((\d+)(?: \+ [A-Za-z &/-]+)?\)$/;
...
function convertToDataObject(_parsedData) {
