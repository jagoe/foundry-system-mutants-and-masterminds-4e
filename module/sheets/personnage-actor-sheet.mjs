...
export class PersonnageActorSheet extends ActorSheet {
    ...
    get template() {
        if (!game.user.isGM && this.actor.limited) {
            return `${templatesPath}/limited-personnage-sheet.html`;
        }
    ...
    _prepareCompetences(context) {
        ...
        keys.forEach((key) => {
            ...
            list[key] = {
                order: isNew ? 'zzzzzz' : game.i18n.localize(CONFIG.MM4.competences[key]),
                label: isNew ? get.label : game.i18n.localize(CONFIG.MM4.competences[key]),
                total: get?.total || 0,
                carac: get?.carac || 0,
                car: get.car,
                rang: get.rang,
    ...
    _prepareSpeed(context) {
        ...
        if (system === 'metric') {
            context.data.system.vitesse.mlabel2 = game.i18n.localize('MM4.VITESSE.Kmh');
            context.data.system.vitesse.mlabel1 = game.i18n.localize('MM4.VITESSE.Metre-short');
        } else {
            context.data.system.vitesse.mlabel2 = game.i18n.localize('MM4.VITESSE.Mph');
            context.data.system.vitesse.mlabel1 = game.i18n.localize('MM4.VITESSE.Pied-short');
            divide = 5280;
        }
        ...
        for (let v in data) {
            ...
            if (autotrade !== false) vData.label = game.i18n.localize(CONFIG.MM4.vitesse[autotrade]);

            if (getSetting('speedcalculate')) {
    ...
    __preparePhisicalDescription(context) {
        const mesureSystem = getSetting('measuresystem');
    ...
    async _onItemCreate(event) {
        ...
        const itemData = {
            ...
            data: data,
            img: {
                equipement: `${assetsPath}/icons/equipement.svg`,
                modificateur: `${assetsPath}/icons/modificateur.svg`,
                pouvoir: `${assetsPath}/icons/pouvoir.svg`,
                qg: `${assetsPath}/icons/qg.svg`,
                talent: `${assetsPath}/icons/talent.svg`,
                vehicule: `${assetsPath}/icons/vehicule.svg`,
            }[type],
        };
    ...
    async _onDrop(event) {
        ...
        const dropType =
            event.target.dataset.type === undefined ? event.target.parentNode.dataset.type : event.target.dataset.type;
        const sort =
            event.target.dataset.sort === undefined ? event.target.parentNode.dataset.sort : event.target.dataset.sort;
