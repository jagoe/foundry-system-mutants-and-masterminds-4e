...
export class editAtk {
    ...
    */
    ...
    get allStatus() {
        const getData = CONFIG.statusEffects
            .map((effect) => ({
                key: effect.id,
    ...
    get html() {
        ...
        const defenses = [
            ...
            {
                key: 'vigueur',
                label: `${game.i18n.localize('MM4.DEFENSE.Vigueur')}`,
            },
            {
                key: 'robustesse',
                label: `${game.i18n.localize('MM4.DEFENSE.Robustesse')}`,
            },
            {
                key: 'volonte',
                label: `${game.i18n.localize('MM4.DEFENSE.Volonte')}`,
            },
        ];
        const options = {
            label: dataAtk.label,
        ...
        if (selectedOptions.includes('typeAtk')) {
            allData.push({
                ...
                list: [
                    {
                        key: 'none',
                        label: `${game.i18n.localize('MM4.ROLL.Typeattaque')} : ${game.i18n.localize('MM4.ROLL.TYPE.Autre')}`,
                    },
                    {
                        key: 'affliction',
                        label: `${game.i18n.localize('MM4.ROLL.Typeattaque')} : ${game.i18n.localize('MM4.ROLL.TYPE.Affliction')}`,
                    },
                    {
                        key: 'dmg',
                        label: `${game.i18n.localize('MM4.ROLL.Typeattaque')} : ${game.i18n.localize('MM4.ROLL.TYPE.Degats')}`,
                    },
                    {
                        key: 'afflictiondmg',
                        label: `${game.i18n.localize('MM4.ROLL.Typeattaque')} : ${game.i18n.localize('MM4.ROLL.TYPE.Affliction')} +  ${game.i18n.localize('MM4.ROLL.TYPE.Degats')}`,
                    },
                ],
            });
        }
        ...
        if (selectedOptions.includes('ability') && this.actor.type === 'personnage') {
            const allAby = CONFIG.MM4.LIST.Caracteristiques;
            let listAby = [];

            for (let aby of allAby) {
                listAby.push({
                    key: aby,
                    label: game.i18n.localize(CONFIG.MM4.caracteristiques[aby]),
                });
            }
            ...
            allData.push(
                {
                    type: 'check',
                    class: this.#dataExist(dataAtk.links.ability)
                        ? 'ability selected btntoggler'
                        : 'ability btntoggler',
                    toggle: 'abytoggler',
                    label: game.i18n.localize('MM4.SPECIAL.AbyLie'),
                },
                {
                    type: 'select',
                    class: this.#dataExist(dataAtk.links.ability)
                        ? 'abychoice abytoggler'
                        : 'abychoice hide abytoggler',
        ...
        if (selectedOptions.includes('skill')) {
            ...
            for (let c in cc) {
                ...
                listSkill.push({
                    key: d._id,
                    type: 'combatcontact',
                    label: `${game.i18n.localize('MM4.COMPETENCES.Combatcontact')} : ${d.label}`,
                });
            }
            ...
            for (let c in rc) {
                ...
                listSkill.push({
                    key: d._id,
                    type: 'combatdistance',
                    label: `${game.i18n.localize('MM4.COMPETENCES.Combatdistance')} : ${d.label}`,
                });
            }
                ...
                allData.push(
                    {
                        ...
                        toggle: 'skilltoggler',
                        label: game.i18n.localize('MM4.SPECIAL.CompLie'),
                    },
                    {
                        type: 'select',
                        class:
                            dataAtk.links.skill === '' ? 'skillchoice hide skilltoggler' : 'skillchoice skilltoggler',
                        selected: dataAtk.links.skill,
                        blank: `- ${game.i18n.localize('MM4.Choisir')} -`,
                        list: listSkill,
                        sort: true,
                    },
                );
                ...
                allData.push(
                    {
                        ...
                        toggle: 'pwrtoggler',
                        label: game.i18n.localize('MM4.SPECIAL.PwrLie'),
                    },
                    {
                        type: 'select',
            ...
            allData.push(
                {
                    ...
                    toggle: 'areatoggler',
                    label: game.i18n.localize('MM4.ROLL.Effetzone'),
                },
                {
                    type: 'input',
                    class: dataAtk.area.has ? 'areaDodge areatoggler' : 'areaDodge hide areatoggler',
                    label: game.i18n.localize('MM4.ROLL.ModEsquive'),
        ...
        if (selectedOptions.includes('noatk')) {
            allData.push({
                ...
                toggle: 'modatk',
                label: game.i18n.localize('MM4.ROLL.Pasattaque'),
            });
        }
        ...
        if (selectedOptions.includes('nocrit')) {
            allData.push({
                ...
                label: game.i18n.localize('MM4.ROLL.Pascritique'),
            });
        }
        ...
        if (selectedOptions.includes('baseDef')) {
            allData.push({
                type: 'input',
                class: 'basedefense',
                label: game.i18n.localize('MM4.ROLL.DEFENSE.Basedefense'),
        ...
        if (selectedOptions.includes('modAtk')) {
            allData.push({
                type: 'input',
                class:
                    this.#dataExist(dataAtk.settings.noatk) ||
                    this.#dataExist(dataAtk.links.skill) ||
                    listSkill.length !== 0
                        ? 'modatk'
                        : 'hide modatk',
                label: game.i18n.localize('MM4.ATTAQUE.ModAttaque'),
        ...
        if (selectedOptions.includes('modEff')) {
            allData.push({
                type: 'input',
                class: 'modeff',
                label: game.i18n.localize('MM4.ATTAQUE.ModEffet'),
        ...
        if (selectedOptions.includes('save')) {
            allData.push({
                ...
                blank: null,
                title: game.i18n.localize('MM4.DEFENSE.Label'),
                list: defenses,
            });
        }
        ...
        if (selectedOptions.includes('defpassive')) {
            allData.push({
                ...
                list: [
                    {
                        key: 'esquive',
                        label: `${game.i18n.localize('MM4.DEFENSE.Jetattaquevs')} ${game.i18n.localize('MM4.DEFENSE.Esquive')}`,
                    },
                    {
                        key: 'parade',
                    ...
                    {
                        key: 'volonte',
                        label: `${game.i18n.localize('MM4.DEFENSE.Jetattaquevs')} ${game.i18n.localize('MM4.DEFENSE.Volonte')}`,
                    },
                    {
                        key: 'vigueur',
                        label: `${game.i18n.localize('MM4.DEFENSE.Jetattaquevs')} ${game.i18n.localize('MM4.DEFENSE.Vigueur')}`,
                    },
                    {
                        key: 'robustesse',
                        label: `${game.i18n.localize('MM4.DEFENSE.Jetattaquevs')} ${game.i18n.localize('MM4.DEFENSE.Robustesse')}`,
                    },
                ],
            });
        }
        ...
        if (selectedOptions.includes('affliction')) {
            allData.push({
                ...
                label: game.i18n.localize('MM4.ROLL.TYPE.Affliction'),
    ...
    async handleDialog() {
        ...
        data.content = await renderTemplate(`${templatesPath}/dialog/edit.html`, this.html);
        data.buttons = {
            one: {
                icon: null,
                label: game.i18n.localize('MM4.SaveEdit'),
                callback: (html) => this.#validate(html),
            },
            two: {
                icon: null,
                label: game.i18n.localize('MM4.Annuler'),
            },
        };
        ...
        new Dialog(
            {
                title: dataAtk.label,
                content: await renderTemplate(`${templatesPath}/dialog/edit.html`, this.html),
                buttons: {
                    one: {
                        icon: null,
                        label: game.i18n.localize('MM4.SaveEdit'),
    ...
    #render(html) {
        ...
        html.find('.repeat div.add a.btn').click(async (ev) => {
            ...
            $(html.find(`div.${type} div.data div.line.l${lvl} div.add select`))
        ...
        html.find('section.body select.skillchoice').change(async (_ev) => {
            this.#toggleModAtt(html);
        });
        ...
        html.find('section.body a.skill.btntoggler').click(async (_ev) => {
            this.#toggleModAtt(html);
        });

        html.find('section.body a.pwr.btntoggler').click(async (_ev) => {
            this.#toggleRangEff(html);
        });

        html.find('section.body a.ability.btntoggler').click(async (_ev) => {
            this.#toggleRangEff(html);
        });
    ...
    #toggleModAtt(html) {
        ...
        if (skill.hasClass('selected') && skillchoice.val() !== '' && !noatk) modatk.removeClass(['hide', 'hidden']);
            ...
            if (type === 'dmg' || type === 'affliction' || (type === 'afflictiondmg' && !modatk.hasClass('hidden')))
                modatk.addClass('hidden');
