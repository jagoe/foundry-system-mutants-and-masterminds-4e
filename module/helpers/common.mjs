import { editAtk } from './editAtk.mjs';
import { getCoreSetting, getSetting } from './foundry.mjs';
...
export const listBg = ['bleuclair', 'violetclair', 'violet', 'bleufonce'];
...
function processChainedAdvantages(pwr) {
    ...
    if (pwr.chainedadvantages?.chainedadvantage !== null) {
...
function processTraitsMod(pwr) {
    ...
    if (pwr?.traitmods?.traitmod !== null) {
...
export async function processPowers(
    actor,
    pouvoirs,
    createItm = true,
    special = 'standard',
    ...
    if (pouvoirs !== null) {
        if (Array.isArray(pouvoirs)) {
            for (let pwr of pouvoirs) {
                ...
                if (pwr.descriptors?.descriptor !== null) {
                ...
                if (createItm) {
                    ...
                    let itm = {
                        name: name[0],
                        type: 'pouvoir',
                        img: `${assetsPath}/icons/pouvoir.svg`,
                        system: {
                    ...
                    const oPwr =
                        link === ''
                            ? await processAlternatePower(actor, pwr, itemCreate, true)
                            : await processAlternatePower(actor, pwr, itemCreate, true);
                    const aPwr =
                        link === ''
                            ? await processAlternatePower(actor, pwr, itemCreate, false)
                            : await processAlternatePower(actor, pwr, itemCreate, false);
                    ...
                    if (totalMod > 0 && !costNull)
                        await itemCreate.update({ [`system.cout.divers`]: itm.system.cout.divers - totalMod });
                    else if (totalMod > 0 && link !== '')
                        await actor.items
                            .get(link)
                            .update({ [`system.cout.divers`]: actor.items.get(link).system.cout.divers - totalMod });

                    const total = itemCreate.system.cout.total;
                    if (total > 1 && (pwr?.alternatepowers?.power ?? null) == null)
                        listPwrWhoCanLostCost.push(itemCreate._id);
            ...
            if (createItm) {
                ...
                const total = itemCreate.system.cout.total;
                if (total > 5 && (pwr?.alternatepowers?.power ?? null) == null)
                    listPwrWhoCanLostCost.push(itemCreate._id);
...
export async function processImport(actor, data, actorType = 'personnage') {
    ...
    if (actorType === 'personnage') {
        for (let attr of attributes) {
            ...
            update[`system.caracteristique.${attributsTRA[attr.name]}.divers`] =
                attr.text === '-' ? 0 : Number(attr.modified) - Number(attr.base);
            update[`system.caracteristique.${attributsTRA[attr.name]}.absente`] =
    ...
    if (actorType === 'personnage') {
        for (let def of defenses) {
            ...
            update[`system.defense.${defensesTRA[def.name]}.divers`] = Math.max(
                Number(def.modified) - (Number(def.cost.value) + totalAttrDef[def.abbr]),
                0,
            );
        }
    } else if (actorType === 'vehicule') {
        ...
        if (ppusedPwr < ppusedPwrActor) {
            for (let pwr of listPwrWhoCanLostCost) {
                ...
                if (resteCout - ppDiff < 1) {
                    await getItm.update({
                        [`system.cout.divers`]: getItm.system.cout.divers - Math.max(resteCout - ppDiff, 1),
                    });
    ...
    if (equipements !== null && actorType === 'personnage') {
        if (Array.isArray(equipements)) {
            for (let eqp of equipements) {
                if (
                    !eqp.name.includes('Dropped to Ground') &&
                    !eqp.name.includes('Grab') &&
                    !eqp.name.includes('Unarmed') &&
                    !eqp.name.includes('Throw')
                ) {
                    ...
                    let itm = {
                        name: eqp.name,
                        type: 'equipement',
                        img: `${assetsPath}/icons/equipement.svg`,
                        system: {
            ...
            if (
                !equipements.name.includes('Dropped to Ground') &&
                !equipements.name.includes('Grab') &&
                !equipements.name.includes('Unarmed') &&
                !equipements.name.includes('Throw')
            ) {
        ...
        if (Array.isArray(equipements)) {
            for (let eqp of equipements) {
                if (
                    ...
                    if (update['system.particularite'] !== undefined)
                        update[`system.particularite`] += `${eqp.name} : ${eqpDdescription}`;
            ...
            if (
                ...
                if (eqpArray !== null) {
                    if (Array.isArray(eqpArray)) {
                        ...
                        egpName = eqpArray.name;
                        eqpDdescription += `<h2>${prcEqpPwrName === '' ? eqpArray.name : prcEqpPwrName}</h2><p>${prcEqpPwrDesc == '' ? eqpArray.description : prcEqpPwrDesc}</p>`;
                        listBonusTalent = listBonusTalent.concat(prcEqpPwr.talents);
                    }

                    if (update['system.particularite'] !== undefined)
                        update[`system.particularite`] += `${egpName} : ${eqpDdescription}`;
                    else update[`system.particularite`] = `${egpName} : ${eqpDdescription}`;
                }
            }
        }
    }
        ...
        if (Array.isArray(talents)) {
            for (let tl of talents) {
                let itm = {
                    ...
                    type: 'talent',
                    img: `${assetsPath}/icons/talent.svg`,
                    system: {
        ...
        for (let skill of skills) {
            ...
            if (
                ...
                if (label.includes('expertise')) {
                    ...
                    listAttack[lengthAttack] = foundry.utils.mergeObject(foundry.utils.deepClone(CONFIG.MM4.StdAtk), {
                        _id: randAtk,
                        links: {
                            skill: randSkill,
                        },
                        ...
                        type: label,
                        label: lastLabel,
                        effet: DCAttacks?.[lastLabel] !== undefined ? DCAttacks[lastLabel] : 0,
                        critique: 20,
                        text: '',
                        isDmg: isDmg,
                ...
                if (label) update[`system.competence.${label}.rang`] = Number(skill.cost.value) * 2;
            }
        }
    }
        ...
        if (Array.isArray(attacks)) {
            for (let att of attacks) {
                if (powerNames.includes(att.name)) {
                    ...
                    if (!alreadyAddAttack.includes(firstName)) {
                        ...
                        listAttack[lengthAttack] = foundry.utils.mergeObject(
                            foundry.utils.deepClone(CONFIG.MM4.StdAtk),
                            {
                                ...
                                effet: Number(powerDetails[att.name]?.ranks),
            ...
            if (powerNames.includes(attacks.name)) {
                ...
                if (!alreadyAddAttack.includes(firstName)) {
                    ...
                    let pwrData = powerDetails[attacks.name];
                    let afflictionechec = [];
                    let save = 'robustesse';
                    let saveAffliction = 'volonte';
                    let basedef = 15;
                    ...
                    listAttack[lengthAttack] = foundry.utils.mergeObject(foundry.utils.deepClone(CONFIG.MM4.StdAtk), {
                        ...
                        type: 'combatcontact',
                        links: {
                            pwr: powerDetails[attacks.name]?._id ?? '',
                        },
                        save: {
                            dmg: {
                                type: save,
                        ...
                        effet: Number(powerDetails[attacks.name]?.ranks),
    ...
    const mesures = getSetting('measuresystem');
    ...
    if (actorType === 'personnage') {
        ...
        update[`system.poids`] =
            mesures === 'metric'
                ? `${Math.round(parseInt(data.personal.charweight.value) * 0.4536)}kg`
                : data.personal.charweight.text;
    ...
    if (actorType === 'vehicule' || actorType === 'qg') {
        ...
        update[`system.cout.particularite`] =
            Number(data.resources.resource[3].spent) +
            Number(data.resources.resource[6].spent) +
            Number(data.resources.resource[7].spent);
    }
...
export function getDices() {
    const optDices = getSetting('typeroll');
...
export async function deletePrompt(actor, label) {
    ...
    let classes = ['dialog', 'mm4-dialog-delete'];

    classes.push(usedFont.replace(' ', ''));
    classes.push(usedSizeFont);

    const confirmation = await Dialog.confirm({
        title: game.i18n.localize('MM4.DIALOG.AskDelete'),
        content: `${label}`,
        options: {
            classes: classes,
        },
...
export function modPromptClasses(actor) {
    ...
    let classes = ['dialog', 'mm4-dialog-mod'];
...
export async function dialogAsk(data = {}) {
    return new Promise((resolve, _reject) => {
        ...
        let html = '';
        if (askDD) {
            title += game.i18n.localize('MM4.DIALOG.AskDD');
            html += `<span>${game.i18n.localize('MM4.DIALOG.DD')}</span><input type="number" class="dc" value="0" min="0"/>`;
        }
        if (askMod) {
            title +=
                title !== ''
                    ? ` / ${game.i18n.localize('MM4.DIALOG.TitleMod')}`
                    : game.i18n.localize('MM4.DIALOG.TitleMod');
            html += `<span>${game.i18n.localize('MM4.DIALOG.AskMod')}</span><input type="number" class="mod" value="0" min="0"/>`;
        }
        ...
        new Dialog(
            {
                ...
                buttons: {
                    button1: {
                        label: game.i18n.localize('MM4.ROLL.Valider'),
                    ...
                    button2: {
                        label: game.i18n.localize('MM4.ROLL.Annuler'),
                        callback: async () => {
                            resolve(0);
                        },
...
export async function rollStd(actor, name, score, dataKey = {}) {
    ...
    const ruleDC = getSetting('dcroll');
    const rMode = getCoreSetting('rollMode');
    ...
    const rollMsgData = {
        ...
        rolls: [roll],
        content: await renderTemplate(`${templatesPath}/roll/std.html`, pRoll),
...
export async function rollVs(actor, name, score, vs, data = {}, dataKey = {}) {
            ...
            if (isDmg && isAffliction) {
                btn.push(
                    ...
                    },
                    {
                        ...
                        vs:
                            Number(dataAtk.save.affliction.effet) +
                            Number(dataStr.effet) +
                            Number(dataAtk.save.affliction.defense),
                    },
                );
            } else if (isDmg) {
                btn.push({
                    ...
                    vs:
                        dataAtk.links.pwr === ''
                            ? Number(dataAtk.save.dmg.effet) + Number(dataStr.effet) + Number(dataAtk.save.dmg.defense)
                            : Number(dataAtk.effet) + Number(dataStr.effet) + Number(dataAtk.save.dmg.defense),
                });
                    ...
                    vs:
                        dataAtk.links.pwr === ''
                            ? Number(dataAtk.save.affliction.effet) +
                              Number(dataStr.effet) +
                              Number(dataAtk.save.affliction.defense)
                            : Number(dataAtk.effet) + Number(dataStr.effet) + Number(dataAtk.save.affliction.defense),
                });
            }
            ...
            btn.push({
                ...
                vs:
                    dataAtk.links.pwr === ''
                        ? Math.max(
                              Math.floor(
                                  Number(dataAtk.save.dmg.effet) / 2 +
                                      Number(dataStr.effet) +
                                      Number(dataAtk.save.dmg.defense),
                              ),
                              1,
                          )
                        : Math.max(
                              Math.floor(
                                  Number(dataAtk.effet) / 2 + Number(dataStr.effet) + Number(dataAtk.save.dmg.defense),
                              ),
                              1,
                          ),
            });
            ...
            btn.push({
                ...
                vs:
                    dataAtk.links.pwr === ''
                        ? Math.max(
                              Math.floor(
                                  Number(dataAtk.save.affliction.effet) / 2 +
                                      Number(dataStr.effet) +
                                      Number(dataAtk.save.affliction.defense),
                              ),
                              1,
    ...
    const saveMsgData = {
        ...
        type: CONST.CHAT_MESSAGE_TYPES.ROLL,
        rolls: [save],
        content: await renderTemplate(`${templatesPath}/roll/std.html`, pRollSave),
        sound: CONFIG.sounds.dice,
    };
...
export async function rollAtkTgt(actor, name, score, data, tgt, dataKey = {}) {
    ...
    const robustesse =
        tokenActor.type === 'vehicule'
            ? Number(tokenData.caracteristique.defense.total)
            : Number(tokenData.defense.robustesse.total) + 10;
    const vigueur =
        tokenActor.type === 'vehicule'
            ? Number(tokenData.caracteristique.defense.total)
            : Number(tokenData.defense.vigueur.total) + 10;
    const volonte =
        tokenActor.type === 'vehicule'
            ? Number(tokenData.caracteristique.defense.total)
    ...
    let formula =
        mod === 0
            ? `${dicesFormula} + ${total} + ${dataStr.attaque}`
            : `${dicesFormula} + ${total} + ${dataStr.attaque} + ${mod}`;
    ...
    switch (defpassive) {
        ...
        case 'vigueur':
            ddDefense = vigueur;
            traType = game.i18n.localize('MM4.DEFENSE.Vigueur');
            break;

        case 'volonte':
            ddDefense = volonte;
            traType = game.i18n.localize('MM4.DEFENSE.Volonte');
            break;
    ...
    if (result.dos >= 1) {
        ...
        if (isArea && tokenActor.type !== 'vehicule') {
            ...
            if (tokenActor.type === 'vehicule') {
                ...
                if (saveAffliction === 'robustesse') {
                    btn.push({
                        ...
                        saveType: saveAffliction,
                        vs:
                            Number(dataCbt.save.affliction.effet) +
                            Number(dataStr.effet) +
                            Number(dataCbt.save.affliction.defense),
                    });
                }
            } else {
                btn.push(
            ...
            isDmg &&
            (tokenActor.type !== 'vehicule' || (tokenActor.type === 'vehicule' && saveType === 'robustesse'))
        ) {
            btn.push({
                typeAtk: 'dmg',
                target: tgt,
                saveType: saveType,
                vs:
                    dataCbt.links.pwr === '' && dataCbt.links.ability === ''
                        ? Number(dataCbt.save.dmg.effet) + Number(dataStr.effet) + Number(dataCbt.save.dmg.defense)
                        : Number(dataCbt.effet) + Number(dataStr.effet) + Number(dataCbt.save.dmg.defense),
            });
        } else if (
            isAffliction &&
            (tokenActor.type !== 'vehicule' || (tokenActor.type === 'vehicule' && saveAffliction === 'robustesse'))
        ) {
            btn.push({
                typeAtk: 'affliction',
                target: tgt,
                ...
                vs:
                    dataCbt.links.pwr === '' && dataCbt.links.ability === ''
                        ? Number(dataCbt.save.affliction.effet) +
                          Number(dataStr.effet) +
                          Number(dataCbt.save.affliction.defense)
                        : Number(dataCbt.effet) + Number(dataStr.effet) + Number(dataCbt.save.affliction.defense),
            });
        }
...
export async function rollTgt(actor, name, data, tgt) {
    ...
    if (isArea) {
        btn.push({
            ...
            vs:
                dataCbt.pwr === ''
                    ? Number(areaBase) + Number(dataStr.effet)
                    : 10 + Number(areaBase) + Number(dataCbt.effet) + Number(dataStr.effet),
        });
    }
...
export async function rollWAtk(actor, name, data) {
    ...
    if (dataCbt.isDmg && dataCbt.links.pwr === '')
        pRoll.effet += Number(dataCbt.save.dmg.effet) + Number(dataStr.effet) + Number(dataCbt.save.dmg.defense);
    else if (dataCbt.isDmg && !dataCbt.isAffliction)
        pRoll.effet += Number(dataCbt.effet) + Number(dataStr.effet) + Number(dataCbt.save.dmg.defense);
    else if (dataCbt.isDmg)
        pRoll.effet += Number(dataCbt.save.dmg.effet) + Number(dataStr.effet) + Number(dataCbt.save.dmg.defense);

    if (pRoll.effet !== '') pRoll.effet += ` / `;

    if (dataCbt.isAffliction && dataCbt.links.pwr === '')
        pRoll.effet +=
            Number(dataCbt.save.affliction.effet) + Number(dataStr.effet) + Number(dataCbt.save.affliction.defense);
    else if (dataCbt.isAffliction && !dataCbt.isDmg)
        pRoll.effet += Number(dataCbt.effet) + Number(dataStr.effet) + Number(dataCbt.save.affliction.defense);
    else if (dataCbt.isAffliction)
        pRoll.effet +=
            Number(dataCbt.save.affliction.effet) + Number(dataStr.effet) + Number(dataCbt.save.affliction.defense);
...
export async function rollAtk(actor, name, score, data, dataKey = {}) {
    ...
    let fRoll =
        mod === 0
            ? `${dicesBase} + ${total} + ${dataStr.attaque}`
            : `${dicesBase} + ${total} + ${dataStr.attaque} + ${mod}`;
...
export async function rollPwr(actor, id, dataKey = {}) {
    ...
    const rollMsgData = {
        ...
        type: CONST.CHAT_MESSAGE_TYPES.ROLL,
        rolls: [roll],
        content: await renderTemplate(`${templatesPath}/roll/pwr.html`, pRoll),
        sound: CONFIG.sounds.dice,
    };
...
export async function sendInChat(actor, itm) {
    ...
    switch (itm.type) {
        case 'talent':
            labelrank = game.i18n.localize('MM4.Rang');
            rank = itm.system.rang;
            break;

        case 'equipement':
            labelrank = game.i18n.localize('MM4.Cout');
    ...
    const msgData = {
        ...
        type: CONST.CHAT_MESSAGE_TYPES.ROLL,
        content: await renderTemplate(`${templatesPath}/roll/msgdata.html`, pData),
        sound: CONFIG.sounds.dice,
    };
...
export function speedCalc(rank) {
    const system = getSetting('measuresystem');
    const used = system === 'metric' ? metric : imperial;
    const knownRanks = Object.keys(used).sort((a, b) => Number(a) - Number(b));
    const maxRank = Number(knownRanks[knownRanks.length - 1]);

    return rank <= maxRank ? used[rank] : used[maxRank] * Math.pow(2, rank - maxRank);
}
...
export function commonHTML(html, origin, data = {}) {
    ...
    if (hasItm) {
        ...
        html.find('.item-delete').click(async (ev) => {
            ...
            let confirm = await deletePrompt(origin, item.name);
            if (!confirm) return;
    ...
    if (hasStr) {
        html.find('a.resetStrategie').click(async (_ev) => {
            const update = {};
    ...
    if (hasRoll) {
        html.find('a.roll').click(async (ev) => {
            if (ev.target.localName !== 'input') {
                ...
                if (isArea == true && game.waitForTemplatePlacementLater) {
                    //the only way to wait for  mm4e-better-attacks to be finish targeting only runs if that module is installed
    ...
    if (hasAdd) {
            ...
            switch (type) {
                ...
                case 'attaque': {
                    ...
                    update[`system.attaque.${maxKeysAtt + 1}`] = foundry.utils.mergeObject(
                        ...
                        {
                            _id: foundry.utils.randomID(),
                            label: game.i18n.localize('MM4.Adefinir'),
                        },
                    );
        ...
        html.find('i.delete').click(async (ev) => {
            ...
            switch (type) {
                ...
                case 'vitesse':
                    confirm = await deletePrompt(
                        origin,
                        origin.system.vitesse.list[id]?.label ?? game.i18n.localize('MM4.Vitesse'),
                    );
                    if (!confirm) return;
    ...
    if (game.user.isGM || actor.ownership.default === 3 || ownership === 3) {
        ...
        if (versioning < 1 || force === true) {
            ...
            for (let i = 0; i < Object.values(atk).length; i++) {
                ...
                update[`system.attaque.${Object.keys(atk)[i]}.save.affliction.type`] =
                    dataAtk?.saveAffliction ?? 'volonte';
...
export async function updateEffects(item, id, name, changes) {
    ...
    if (actor !== null && isVersion12()) {
        const getActorEffects = actor.effects.contents.find(
            (itm) => (itm?.origin?.includes(item._id) ?? false) && itm.getFlag(systemId, 'variante') === name,
        );
...
export async function updateVarianteName(item, id, variante, name) {
    ...
    if (actor !== null && isVersion12()) {
        const getActorEffects = actor.effects.contents.find(
            (itm) => (itm?.origin?.includes(item._id) ?? false) && itm.getFlag(systemId, 'variante') === variante,
        );
...
export function deleteEffects(item, id, name) {
    ...
    if (actor !== null && isVersion12()) {
        const getActorEffects = actor.effects.find(
            (itm) => itm.origin.includes(item._id) && itm.getFlag(systemId, 'variante') === name,
        );
...
export function loadEffectsContext(context) {
    const mods = CONFIG.MM4.listmods;
    let effects = {};
    ...
    for (let m in mods) {
        ...
        if (m.includes('caracteristique')) {
            label = `${game.i18n.localize('MM4.Caracteristique')} : `;
        } else if (m.includes('defense')) {
            label = `${game.i18n.localize('MM4.DEFENSE.Label')} : `;
        } else if (m.includes('competence')) {
            label = `${game.i18n.localize('MM4.Competence')} : `;
        } else if (m.includes('strategie')) {
            label = `${game.i18n.localize('MM4.STRATEGIE.Limite')} : `;
        } else {
            label = `${game.i18n.localize('MM4.Autre')} : `;
        }
    ...
    context.item.listSurcharge = {
        surcharge: 'MM4.EFFECTS.Surcharge',
        surchargeranks: 'MM4.EFFECTS.SurchargeRanks',
        ranks: 'MM4.EFFECTS.Ranks',
    };
}
...
export function loadEffectsHTML(html, item, permanent = false, single = false) {
    ...
    if (single) {
        html.find(`i.addMod`).click(async (ev) => {
            ...
            if (size === 0) {
                ...
                effect = item.effects.find((itm) => itm.getFlag(systemId, 'variante') === 'e0');
                ...
                await updateEffects(item, effect._id, effect.getFlag(systemId, 'variante'), changes);
            }
        });
        ...
        html.find(`input.varianteName`).blur((ev) => {
            ...
            if (size !== 0) {
                const effect = item.effects.find((itm) => itm.getFlag(systemId, 'variante') === 'e0');

                updateVarianteName(item, effect._id, effect.getFlag(systemId, 'variante'), target.val());
            }
        });
    } else {
        html.find(`input.varianteName`).blur((ev) => {
        ...
        html.find(`i.addVMod`).click((ev) => {
            ...
            const name = `${game.i18n.localize('MM4.EFFECTS.Variante')} ${max + 1}`;
            let update = {};
    ...
    html.find(`i.deleteMod`).click((ev) => {
        ...
        updateEffects(item, id, effect.getFlag(systemId, 'variante'), effect.changes);
    });
...
export async function loadEffectsClose(item) {
    ...
    for (let i of input) {
        ...
        const value = $(i).val();
        const effect = item.effects.find((itm) => itm._id === id && itm.getFlag(systemId, 'variante') === variante);
    ...
    for (let i of variantes) {
        ...
        if (single) {
            ...
            if (size !== 0) {
                ...
                updateVarianteName(item, effect._id, effect.getFlag(systemId, 'variante'), value);
            }
        } else {
            const id = $(i).data('id');
            const variante = $(i).data('name');
