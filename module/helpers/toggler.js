class Toggler {
    ...
    init(id, html) {
        ...
        html.find('a.closeAll').click((_ev) => {
            this.closeAll(id, html);
        });

        html.find('a.openAll').click((_ev) => {
            this.openAll(id, html);
        });
    }

    closeAll(id, html) {
        html[0].querySelectorAll('.js-toggler').forEach((element, _index) => {
    ...
    _save() {
        ...
        localStorage.setItem('mm4.togglers', JSON.stringify(dataset));
    }

    _load() {
        const dataset = localStorage.getItem('mm4.togglers');
        if (null === dataset) {
            return new Map();
        }
