...
export class PersonnageDataModel extends foundry.abstract.TypeDataModel {
    ...
    static defineSchema() {
        ...
        for (let b of CONFIG.MM4.LIST.Base) {
            base[b] = new StringField({ initial: '' });
        }

        for (let c of CONFIG.MM4.LIST.Caracteristiques) {
        ...
        for (let s of CONFIG.MM4.LIST.Strategie) {
            ...
            limite[s] = new SchemaField({
                atk: new SchemaField({
                    base: new NumberField({ initial: CONFIG.MM4.LIST.LimiteStrategie[s].attaque }),
                    bonuses: new NumberField({ initial: 0 }),
                    surcharge: new NumberField({ initial: 0 }),
                    ranks: new ObjectField(),
                    surchargeranks: new ObjectField(),
                ...
                eff: new SchemaField({
                    base: new NumberField({ initial: CONFIG.MM4.LIST.LimiteStrategie[s].effet }),
                    bonuses: new NumberField({ initial: 0 }),
                    surcharge: new NumberField({ initial: 0 }),
                    ranks: new ObjectField(),
                    surchargeranks: new ObjectField(),
                ...
                def: new SchemaField({
                    base: new NumberField({ initial: CONFIG.MM4.LIST.LimiteStrategie[s].defense }),
                    bonuses: new NumberField({ initial: 0 }),
                    surcharge: new NumberField({ initial: 0 }),
                    ranks: new ObjectField(),
                    surchargeranks: new ObjectField(),
                ...
                attaque: new NumberField({ initial: CONFIG.MM4.LIST.LimiteStrategie[s].attaque }),
                defense: new NumberField({ initial: CONFIG.MM4.LIST.LimiteStrategie[s].defense }),
                effet: new NumberField({ initial: CONFIG.MM4.LIST.LimiteStrategie[s].effet }),
            });
        }
    ...
    #_strValue() {
        ...
        for (let s of CONFIG.MM4.LIST.Strategie) {
            ...
            for (let r in surchargeAtkRangs) {
                ...
                if (itm && itm.type === 'pouvoir') {
                    if (itm.system.special === 'dynamique') {
                        surchargeAtkRangsValue = Math.max(
                            itm.system.cout.rangDyn * surchargeAtkRangs[r],
                            surchargeAtkRangsValue,
                        );
                    } else {
                        surchargeAtkRangsValue = Math.max(
                            itm.system.cout.rang * surchargeAtkRangs[r],
                            surchargeAtkRangsValue,
                        );
                    }
                } else if (itm) {
            ...
            for (let r in surchargeEffRangs) {
                ...
                if (itm && itm.type === 'pouvoir') {
                    if (itm.system.special === 'dynamique') {
                        surchargeEffRangsValue = Math.max(
                            itm.system.cout.rangDyn * surchargeEffRangs[r],
                            surchargeEffRangsValue,
                        );
                    } else {
                        surchargeEffRangsValue = Math.max(
                            itm.system.cout.rang * surchargeEffRangs[r],
                            surchargeEffRangsValue,
                        );
                    }
                } else if (itm) {
            ...
            for (let r in surchargeDefRangs) {
                ...
                if (itm && itm.type === 'pouvoir') {
                    if (itm.system.special === 'dynamique') {
                        surchargeDefRangsValue = Math.max(
                            itm.system.cout.rangDyn * surchargeDefRangs[r],
                            surchargeDefRangsValue,
                        );
                    } else {
                        surchargeDefRangsValue = Math.max(
                            itm.system.cout.rang * surchargeDefRangs[r],
                            surchargeDefRangsValue,
                        );
                    }
                } else if (itm) {
            ...
            const tAtk = isSurcharge(
                Math.max(limite.atk.surcharge, surchargeAtkRangsValue),
                limite.atk.base,
                limite.atk.allbonuses,
            );
            const tDef = isSurcharge(
                Math.max(limite.def.surcharge, surchargeDefRangsValue),
                limite.def.base,
                limite.def.allbonuses,
            );
            const tEff = isSurcharge(
                Math.max(limite.eff.surcharge, surchargeEffRangsValue),
                limite.eff.base,
                limite.eff.allbonuses,
            );
        ...
        for (let c of CONFIG.MM4.LIST.Caracteristiques) {
            ...
            if (carac.absente) {
                ...
                for (let r in surchargeRanks) {
                    ...
                    if (itm) {
                        if (itm.system.special === 'dynamique') {
                            surchargeRanksValue = Math.max(
                                itm.system.cout.rangDyn * surchargeRanks[r],
                                surchargeRanksValue,
                            );
                        } else {
                            surchargeRanksValue += Math.max(
                                itm.system.cout.rang * surchargeRanks[r],
                                surchargeRanksValue,
                            );
                        }
                    }
    ...
    #_defense() {
        ...
        for (let d of CONFIG.MM4.LIST.Defenses) {
            ...
            for (let r in ranks) {
                ...
                if (itm) {
                    if (itm.system.special === 'dynamique') {
                        const rDyn =
                            itm.type === 'pouvoir'
                                ? parseInt(itm.system.cout.rangDyn)
                                : parseInt(itm.system?.rang ?? 1);

                        ranksValue += rDyn * ranks[r];
                    } else {
                        const ra =
                            itm.type === 'pouvoir' ? parseInt(itm.system.cout.rang) : parseInt(itm.system?.rang ?? 1);
            ...
            if (!currentDefense.defenseless)
                total = isSurcharge(
                    Math.max(currentDefense.surcharge, surchargeRanksValue),
                    defRang,
                    currentDefense.carac,
                    currentDefense.divers,
                    mod,
                    currentDefense.bonuses,
                    ranksValue,
                );
    ...
    #_initiative() {
        ...
        Object.defineProperty(initiative, 'total', {
            value: isSurcharge(
                Math.max(initiative.surcharge, surchargeRanksValue),
                initiative.carac,
                initiative.base,
                initiative.bonuses,
                ranksValue,
    ...
    #_equipment() {
        const coutEqp = this.items
            .filter((item) => item.type === 'equipement')
            .reduce((acc, item) => acc + item.system.cout, 0);
        const ptsEqp = this.items
            .filter((item) => item.type === 'talent' && item.system.equipement)
            .reduce((acc, item) => acc + item.system.rang * 5, 0);
    ...
    #_vitesse() {
        ...
        Object.defineProperty(this.vitesse, 'actuel', {
            value: getSetting('speedcalculate') ? find.rang : find.round,
        });
    }
