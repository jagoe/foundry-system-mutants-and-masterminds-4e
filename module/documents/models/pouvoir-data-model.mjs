...
export class PouvoirDataModel extends foundry.abstract.TypeDataModel {
    ...
    #_dyn() {
        ...
        if (actor && isDynamique) {
            ...
            if (rangDyn) {
                ...
                if (coutParRang > 0)
                    rangDyn.cout.actuel = Number(rang) * cout.parrangtotal + cout.divers + cout.modfixe - 1;
                else if (coutParRang === 0)
                    rangDyn.cout.actuel = Math.floor(Number(rang) / 2 + (cout.divers + cout.modfixe - 1));
                else if (coutParRang < 0)
                    rangDyn.cout.actuel = Math.floor(
                        Number(rang) / (coutParRang * -1 + 2) + (cout.divers + cout.modfixe - 1),
                    );
    ...
    async #_effects() {
        ...
        const variante =
            this.effectsVarianteSelected === ''
                ? Object.keys(this.listEffectsVariantes)[0]
                : this.effectsVarianteSelected;
        const effectsToUpdate = [];

        await this.#_updateV13(effects);

        const effectItem = this.effects.find(
            (itm) => itm.getFlag(systemId, 'variante') === variante && itm.disabled !== nactive,
        );
        const effectsItem = this.effects.filter(
            (itm) => itm.getFlag(systemId, 'variante') !== variante && itm.disabled !== true,
        );
        ...
        if (this.isVersion12) {
            ...
            const actorToUpdate = [];
            const actorItem = actor.effects.find(
                (itm) =>
                    (itm?.origin?.includes(this.item.id) ?? false) &&
                    itm.getFlag(systemId, 'variante') === variante &&
                    itm.disabled !== nactive,
            );
            const actorItems = actor.effects.filter(
                (itm) =>
                    (itm?.origin?.includes(this.item._id) ?? false) &&
                    itm.getFlag(systemId, 'variante') !== variante &&
                    itm.disabled !== true,
            );
    ...
    async #_updateV13(effects) {
        ...
        for (let e of filter) {
            ...
            effectsToUpdate.push({
                _id: e.id,
                [`flags.${systemId}.variante`]: keysVariante.find((key) => listVariante[key] === e.name),
            });
        }
    ...
    calculateLinkedPwrCostUsed(id, excludeMain = true) {
        const actor = this.actor;
        const filter = excludeMain
            ? actor.items.filter((itm) => itm.system.link === id)
            : actor.items.filter((itm) => (itm.system.link === id || itm.id === id) && itm.id !== this.item.id);
